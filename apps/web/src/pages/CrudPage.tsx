...
export function CrudPage({
  ...
  const [formOpen, setFormOpen] = useState(
    !startFormCollapsed || formPlacement !== "top",
  );
  ...
  const resolvedFormTitle = useMemo(() => {
    if (editingId) {
      return (
        editTitle ??
        (title === "Clientes" ? "Alterar ficha do cliente" : "Alterar registro")
      );
    }

    return (
      createTitle ??
      (title === "Clientes" ? "Incluir cliente" : "Incluir registro")
    );
  ...
  function formFromItem(item: Record<string, unknown>) {
    return fields.reduce<Record<string, FieldValue>>(
      (acc, field) => {
        ...
        const value =
          valueFromPath(item, field.source ?? field.name) ??
          valueFromPath(item, field.name);
          ...
          <div className="rounded-2xl border border-line bg-white p-3">
                ...
                (form[field.name] as string[]).map((selectedValue: string) => {
                  ...
                  return (
                    <button
                      ...
                      onClick={() =>
                        toggleMultiselectValue(field.name, selectedValue)
                      }
                    >
            ...
            <div className="max-h-56 space-y-2 overflow-y-auto pr-1">
              {filteredOptions.map((option) => {
                const selected =
                  Array.isArray(form[field.name]) &&
                  form[field.name].includes(option.value);
                ...
                return (
                  <button
                    ...
                    onClick={() =>
                      toggleMultiselectValue(field.name, option.value)
                    }
                  >
                    {option.label}
                  </button>
          ...
          <div className="rounded-2xl border border-line bg-white p-3">
            <div className="mb-3 flex flex-wrap gap-2">
              ...
              form[field.name].length > 0 ? (
                ...
                <span className="text-sm font-semibold text-stone-500">
                  Nenhuma atividade nova adicionada.
                </span>
              )}
            </div>
            ...
            <div className="flex flex-col gap-3 sm:flex-row">
              <input
                ...
                type="text"
                placeholder={
                  field.placeholder ?? "Digite a atividade e confirme"
                }
  ...
  function renderForm() {
    const formClass = isTopForm
      ? "panel overflow-hidden"
      : "panel overflow-hidden 2xl:sticky 2xl:top-20 2xl:self-start";
    const bodyClass = isTopForm
      ...
      <form onSubmit={onSubmit} className={formClass}>
        <div className="panel-header">
          <div>
            <div className="mb-2">
              <span
                className={`${editingId ? "execution-chip" : "brand-chip"}`}
              >
        ...
        <div className={bodyClass}>
          ...
          <div className="flex flex-col-reverse gap-3 border-t border-line pt-5 sm:flex-row sm:justify-end">
            ...
            <button
              ...
              title={actionLabel}
              disabled={loading}
              className="primary-button justify-center sm:min-w-56"
            >
              {editingId ? (
        ...
        <div className="border-b border-line/80 bg-white/90 p-4">
          ...
          <div className="grid gap-3 md:grid-cols-[minmax(0,1fr)_150px]">
            ...
            <div className="flex items-center justify-center rounded-2xl border border-line bg-field px-4 py-3 text-sm font-semibold text-slateText">
              Exibindo{" "}
              <span className="ml-1 font-black text-ink">
                    ...
                    <div className="flex flex-wrap gap-2">
                      ...
                      <button
                        ...
                        onClick={() => {
                          if (
                            window.confirm(
                              "Deseja excluir/inativar este registro?",
                            )
                          ) {
  ...
  return (
    <section>
      ...
      {isTopForm ? (
        ...
        <div
          className={`grid gap-4 ${canCreate || editingId ? (formMode === "drawer" ? "2xl:grid-cols-[minmax(0,1fr)_400px]" : "2xl:grid-cols-[minmax(0,1fr)_340px]") : ""}`}
        >
          {renderTable()}
