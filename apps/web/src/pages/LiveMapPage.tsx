...
const LIVE_MAP_REFRESH_INTERVAL_MS = 10 * 1000;
...
function statusPriority(status: LivePromoter["status"]) {
  switch (status) {
    case "online":
      return 0;
    case "stale":
...
function operationalLabel(item: LivePromoter) {
  if (item.activeVisit) {
...
function accuracyLabel(location?: LivePromoter["location"] | null) {
...
export function LiveMapPage() {
  ...
  const bounds = useMemo(() => buildBounds(orderedItems), [orderedItems]);
  const locatedItems = orderedItems.filter((item) => item.location?.latitude !== null && item.location?.longitude !== null);
  const onlineCount = orderedItems.filter((item) => item.status === "online").length;
  const activeCount = orderedItems.filter((item) => item.activeVisit).length;
  const routeCount = orderedItems.filter((item) => item.activeRoute).length;

  return (
    <section>
      <PageHeader
        title="Mapa ao vivo"
        subtitle="Acompanhe em tempo quase real o deslocamento online dos promotores durante rota publicada ou atendimento em andamento."
        action={
      ...
      <div className="mb-5 grid gap-3 sm:grid-cols-2 xl:grid-cols-4">
        ...
        <div className="metric-card">
          ...
          <div className="relative z-[1] mt-1 text-xs font-bold text-stone-500">A cada 10 segundos</div>
        </div>
      </div>
              ...
              <div className="absolute inset-0 grid place-items-center px-6 text-center">
                <div className="max-w-md rounded-3xl border border-white/80 bg-white/90 p-7 shadow-xl backdrop-blur">
                  ...
                  <p className="mt-2 text-sm font-semibold leading-6 text-stone-500">
                    O mapa sera preenchido quando um promotor abrir o aplicativo durante a jornada autorizada e enviar a localizacao.
                  </p>
                </div>
              </div>
            )}
        ...
        <div className="table-wrap">
          <div className="panel-header">
            <div>
              ...
              <p className="panel-subtitle">Ultima localizacao recebida do aplicativo durante a jornada ativa.</p>
            </div>
          </div>
              ...
              <div key={item.promoter.id} className="border-b border-line/70 p-4 last:border-b-0">
                ...
                <div className="mt-4 rounded-2xl bg-field p-3 text-sm">
                  ...
                  <div className="mt-1 text-xs font-semibold text-stone-500">
                    {item.activeVisit
                      ? `Rota da visita: ${item.activeVisit.routeName ?? "rota sem nome"}`
                      : item.activeRoute
                        ? `Janela operacional: ${formatTime(item.activeRoute.startDate ?? item.activeRoute.scheduledDate)} ate ${formatTime(item.activeRoute.endDate)}`
                        : "Sem roteiro ativo"}
                  </div>
                  <div className="mt-1 text-xs font-semibold text-stone-500">
                ...
                {item.location ? (
                  <a className="secondary-button mt-3 h-10 w-full" href={mapsUrl(item.location)} target="_blank" rel="noreferrer">
