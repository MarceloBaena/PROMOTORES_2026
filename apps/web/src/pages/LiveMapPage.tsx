...
function PromoterOperationCard({
  ...
  return (
    <article
      className={`bg-white px-3 py-3 transition ${selected ? "relative z-[1] ring-2 ring-brand" : "hover:bg-field/70"}`}
    >
      <div
        role="button"
        tabIndex={0}
      ...
      >
        <div className="flex items-start justify-between gap-2">
          <div className="flex min-w-0 items-start gap-2.5">
            <div className="relative h-11 w-11 shrink-0">
              <div className="grid h-11 w-11 place-items-center overflow-hidden rounded-full bg-gradient-to-br from-slate-200 to-blue-100 text-xs font-black text-navy ring-1 ring-line">
            ...
            <div className="min-w-0">
              <div className="truncate text-[13px] font-black uppercase tracking-tight text-ink">
                {item.promoter.name}
              </div>
              ...
              <div className="mt-1 truncate text-[11px] font-black uppercase text-brand">
        ...
        <div className="mt-3 space-y-1.5">
          ...
          <div className="truncate text-[13px] font-black uppercase text-brand">
            {latestPlace}
          </div>
          <div className="text-xs font-semibold text-slateText">
            {promoterCode(item.promoter.code)} - Ponto
          </div>
          <div className="flex flex-wrap items-center gap-2 text-[11px] font-bold text-slateText">
            <span className="inline-flex items-center gap-1">
              <BatteryMedium
                className={`h-3.5 w-3.5 ${item.status === "online" ? "text-execution" : "text-stone-400"}`}
              />
        ...
        {!hasRoute ? (
          <Link
            to="/roteirizacao"
            className="mt-3 inline-flex h-9 items-center justify-center rounded-xl border border-brand/40 bg-white px-3 text-xs font-black text-brand no-underline transition hover:bg-blue-50"
            onClick={(event) => event.stopPropagation()}
          >
            Adicionar roteiro
          </Link>
        ) : (
          <div className="mt-3 rounded-xl bg-field px-3 py-2">
            <div className="flex items-center justify-between gap-3 text-[11px] font-black text-slateText">
              <span>Roteiro</span>
              <span>{progress}%</span>
            </div>
...
function DualProgress({
  ...
  return (
    <div className={className}>
      <div className="mb-1 flex items-center justify-between text-[11px] font-black text-slateText">
        <span>{label}</span>
        <span>
          {done} / {total}
        </span>
